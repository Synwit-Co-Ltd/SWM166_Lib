...
void Serial1Init(void)
{
	...
	GPIO_Init(GPIOA, PIN4, 1, 0, 0, 0);		//用于产生长break
	GPIO_ClrBit(GPIOA, PIN4);
	
	PORT_Init(PORTA, PIN5, PORTA_PIN5_UART1_RX, 1);	//GPIOA.5配置为UART1输入引脚
	PORT_Init(PORTA, PIN4, PORTA_PIN4_UART1_TX, 0);	//GPIOA.4配置为UART1输出引脚
